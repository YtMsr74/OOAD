...
namespace patlab4
{
    public partial class Form1 : Form
    {
        ...
        private void pictureBox_Click(object sender, EventArgs e)
        {
            ...
            Vehicle selected = GetVehicle(Convert.ToInt32(pictureBox.Tag), _connection);
        ...
        private Vehicle GetVehicle(int id, SQLiteConnection con)
        {
            using (SQLiteCommand com = new SQLiteCommand("SELECT * FROM vehicles WHERE id = @id", con))
            {
                ...
                using (SQLiteDataReader reader = com.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        Vehicle veh = new Vehicle(reader.GetInt32(0), reader.GetString(1), reader.GetInt32(2), reader.GetString(3), reader.GetString(4), reader.GetString(5), reader.GetString(6), reader.GetString(7) + ".jpg");
                        return veh;
                    }
                }
            }
